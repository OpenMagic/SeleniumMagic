...
Function Get-PackageFolder($packagesFolder, $filter)
{
    Write-Host $packagesFolder
    $packageFolder = Get-ChildItem -Path $packagesFolder -Directory -Filter $filter
    
    If ($packageFolder -eq $null)
    {
        throw "Cannot find package folder '$filter' in '$packagesFolder'."
    }
    
    Return $packageFolder.FullName
}
